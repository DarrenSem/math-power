...
module.exports = (test) => {
  ...
  test.it("Gets calctest function from calc.js", (assert, _) => {
    _= typeof calctest;
    assert(_ === "function", `Expected calctest to be a |function|, got |${_}|`);
  } );
  ...
  test.it("Defaults to CLI args/querystring if passed null or all whitespace", (assert, _) => {
    ...
    // 12
    _ = calctest(null);
    assert(_ === 12, `Expected calctest(null) to be |12|, got |${_}|`);
    ...
    // 12
    _ = calctest();
    assert(_ === 12, `Expected calctest() to be |12|, got |${_}|`);
  ...
  test.it("Aborts if missing operator", (assert, _) => {
    ...
    assert(_ === undefined, `Expected result to be |undefined|, got |${_}|`);
    ...
    assert(_ === undefined, `Expected calctest({ num1: 7, operator: "timey" }) to be |undefined|, got |${_}|`);
    ...
    // undefined
    _ = calctest({});
    assert(_ === undefined, `Expected calctest({}) to be |undefined|, got |${_}|`);
    ...
    // undefined
    _ = calctest("5 7 9");
    assert(_ === undefined, `Expected calctest("5 7 9") to be |undefined|, got |${_}|`);

  } );

  test.it("Returns NaN if missing number(s) with valid operator", (assert, _) => {
    ...
    assert(isNaN(_), `Expected calctest({ num2: 5, operator: "times" }) to be |NaN|, got |${_}|`);
    ...
    assert(isNaN(_), `Expected calctest("num2: 3, operator: minus") to be |NaN|, got |${_}|`);
    ...
    assert(isNaN(_), `Expected calctest("num2 4, operator=times") to be |NaN|, got |${_}|`);

  } );

  test.it("Returns correct numeric answer if provided both numbers with valid operator", (assert, _) => {
    ...
    assert(_ === 42, `Expected calctest({ num1: 7, num2: 6, operator: "times" }) to be |42|, got |${_}|`);
    ...
    // 13
    _ = calctest("5 plus 8");
    assert(_ === 13, `Expected calctest("5 plus 8") to be |13|, got |${_}|`);
    ...
    assert(_ === 18.1, `Expected calctest("7.1,minus,-11") to be |18.1|, got |${_}|`);
    ...
    assert(_ === 10, `Expected calctest([4, "times", 2.5]) to be |10|, got |${_}|`);
